    ...
    return (
        <div>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Mecánicos</h1>
                    <p className="text-slate-500 dark:text-slate-400">Directorio de mecánicos y talleres de confianza.</p>
                </div>
                <button
                    onClick={handleAdd}
            ...
            {mechanics.length === 0 ? (
                <div className="bg-white dark:bg-slate-900 p-12 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 text-center transition-colors duration-300">
                    ...
                    <h3 className="text-lg font-medium text-slate-800 dark:text-white mb-2">No hay mecánicos registrados</h3>
                    <p className="text-slate-500 dark:text-slate-400 mb-6">Agrega tus mecánicos de confianza.</p>
                    <button
                        onClick={handleAdd}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredMechanics.map((mechanic) => (
                        <div key={mechanic.id} className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 hover:shadow-md transition-all group">
                            <div className="flex justify-between items-start mb-4">
                                <div className="flex items-center gap-3">
                                    <div className="w-12 h-12 bg-purple-50 dark:bg-purple-900/20 rounded-xl flex items-center justify-center text-purple-600 dark:text-purple-400 font-bold text-lg">
                                        <Wrench className="w-6 h-6" />
                                    </div>
                                    <div>
                                        <h3 className="text-lg font-bold text-slate-800 dark:text-white">{mechanic.nombre}</h3>
                                        <p className="text-slate-500 dark:text-slate-400 text-sm">{mechanic.especialidad || 'General'}</p>
                                    </div>
                                </div>
                            ...
                            {mechanic.notas && (
                                <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                                    <p className="text-sm text-slate-500 dark:text-slate-400 italic">"{mechanic.notas}"</p>
                                </div>
                            )}
                        </div>
                    ))}
