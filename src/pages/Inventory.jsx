        ...
        <div>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    ...
                    <p className="text-slate-500">Control de repuestos y stock.</p>
                </div>
                <button
                    onClick={handleAdd}
            ...
            {inventory.length === 0 ? (
                <div className="bg-white p-12 rounded-2xl shadow-sm border border-slate-100 text-center">
                    ...
                    <h3 className="text-lg font-medium text-slate-800 mb-2">Inventario vacío</h3>
                    <p className="text-slate-500 mb-6">Agrega repuestos para llevar el control.</p>
                    <button
                        onClick={handleAdd}
                        ...
                        <div key={item.id} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow group relative overflow-hidden">
                            ...
                            <div className="flex justify-between items-start mb-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-slate-500 text-sm">{item.numero_parte || 'S/N'}</p>
                                    </div>
                                </div>
                            ...
                            <div className="grid grid-cols-2 gap-4 mt-4">
                                <div className="bg-slate-50 p-3 rounded-xl">
                                    <p className="text-xs text-slate-500 uppercase font-semibold mb-1">CANTIDAD</p>
                                    <p className={`text-lg font-bold ${item.cantidad < 5 ? 'text-red-600' : 'text-slate-800'}`}>
                                        {item.cantidad}
                                    </p>
                                </div>
                                <div className="bg-slate-50 p-3 rounded-xl">
                                    <p className="text-xs text-slate-500 uppercase font-semibold mb-1">PRECIO</p>
                                    <p className="text-lg font-bold text-slate-800">${item.precio}</p>
                                </div>
                            </div>
